...
bool    ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags)
{
    ...
    ImRect tab_bar_bb = ImRect(window->DC.CursorPos.x, window->DC.CursorPos.y, window->WorkRect.Max.x , window->DC.CursorPos.y + g.FontSize + g.Style.FramePadding.y *2.1);

    ImU32 fill_color = IM_COL32(0, 0, 0, 255); // Example: A blue rectangle

    // Draw a filled rectangle covering the tab bar bounding box
    window->DrawList->AddRectFilled(ImVec2(tab_bar_bb.Min.x + 1.4f, tab_bar_bb.Min.y + 1.4f), ImVec2(tab_bar_bb.Max.x, tab_bar_bb.Max.y), fill_color,  10.f );
...
bool ImGui::Toggle(const char* text, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 label_size = CalcTextSize(text, NULL, false);

    float height = 18.f;
    const ImVec2 pos = window->DC.CursorPos;

    float width = height * 2;  // the width of the toggle

    ImDrawList* draw_list = ImGui::GetForegroundDrawList();
    draw_list->Flags |= ImDrawListFlags_AntiAliasedFill | ImDrawListFlags_AntiAliasedLines;
    const ImRect total_bb(pos, pos + ImVec2(ImGui::GetContentRegionAvail().x , height * 2 )); // the total bounding box of the whole section
    draw_list->AddRectFilled(total_bb.Min, total_bb.Max, ImColor(1.f, 1.f, 1.f, 0.04f), 10.f);
    ItemSize(total_bb, 0);
    if (!ItemAdd(total_bb, id))
        return false;
    ...
    {
        float t_anim = ImSaturate(g.LastActiveIdTimer / 0.06f);
    ...
    ImU32 col_bg = ImColor(153, 153, 153, 255);

    if (*v)
        col_bg = ImColor(0, 122, 204, 255);

    if (g.LastActiveIdTimer == 0.f && g.LastActiveId == id && !pressed)
        g.LastActiveIdTimer = last_active_id_timer;

    RenderFrame(frame_bb.Min, frame_bb.Max, col_bg, true, style.FrameRounding); // Set the radius to 0 to make it squared
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if (is_visible)
    {
        ...
        if (tab_bar && tab_bar->SelectedTabId == tab->ID)
        {
            if (style.TabRounding > 0.0f)
            {
                float rounding = style.TabRounding;

                // 🔵 Bottom-Left Arc (0 → 3) ⬅️⬇️
                display_draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding, bb.Max.y - rounding), rounding, 0, 3);
        ...
        // Compute centered position **relative to the tab's bounding box (bb)**
        ImVec2 text_pos = ImVec2(
            (bb.GetWidth() - text_size.x) * 0.5f,  // Center horizontally within tab
            (bb.GetHeight() - text_size.y) * 0.5f  // Center vertically within tab
        );

        // Call function with the correct relative position
        TabItemLabelAndCloseButton(display_draw_list, bb,
            tab_just_unsaved ? (flags & ~ImGuiTabItemFlags_UnsavedDocument) : flags,
            text_pos, label, id, close_button_id, tab_contents_visible,
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImU32 col)
{
    ...
    draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding, y2 - rounding), rounding, 0, 3);
    draw_list->PathArcToFast(ImVec2(bb.Min.x + rounding, y2 - rounding), rounding, 3, 6);
