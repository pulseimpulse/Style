...
static void ImGui_ImplDX12_CreateWindow(ImGuiViewport* viewport)
{
    ...
    if (FAILED(bd->pd3dDevice->CheckFeatureSupport(D3D12_FEATURE_MULTISAMPLE_QUALITY_LEVELS, &msaaQualityLevels, sizeof(msaaQualityLevels))) ||
        msaaQualityLevels.NumQualityLevels == 0)
    {
        sampleCount = 1; // No MSAA fallback
    }
    else
    {
        msaaQuality = msaaQualityLevels.NumQualityLevels - 1; // Use highest quality level
    }
    ...
    sd1.SampleDesc.Count = sampleCount;
    sd1.SampleDesc.Quality = msaaQuality;
